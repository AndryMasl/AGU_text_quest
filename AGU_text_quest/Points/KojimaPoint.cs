...
namespace Points
{
	...
	{
		public override string Content => $"Кодзима вещает своим слушателям что-то возвышенной, он утверждает, что придумал новый способ взаимодействия со зрителем, но в челом нифина не понятно, потому что он говорит на японском. И слушатели тоже нифига не понимают но делают многозначительный вид и говорят: Угу, ада, да, это гениально...";
	...
	internal class FifthActionKojimaPoint : FifthAction
	{
		...
		private void DoAfterActionLocal(Player player)
		{
			...
			Console.WriteLine($"{Player.Instance.Name} запрыгивает на стол перед Кодзимой и кричит: Кодзима гений!!! Все вокруг начинают хлопать. Кодзима улыбается и дает {Player.Instance.Name}у свою везитку. {Player.Instance.Name} добился того, чего хотел, теперь можно возвращаться к своим делам.\n");
		}
	}
	...
	internal class SecondActionKojimaPoint : SecondAction
	{
        ...
        {
			IsAvailable = false;
			MassageAfterAction = $"Кодзима прерывает свой монолог, дает {Player.Instance.Name}у автограв, затем продолжает говорить: 魚のスープ魚のスープ魚のスープ...\n";
        }
    }
