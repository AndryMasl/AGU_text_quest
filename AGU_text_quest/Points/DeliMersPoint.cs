...
namespace Points
{
	...
	{
		public override string Content => $"Пора сваливать!!! {Player.Instance.Name} выбел ногой окно и ломанулся на крышу, Цой за ним. Со всех сторон летают бронированные ави, один из дронов заметил нувизучих похитителей и открыл огонь из крупноколиберного пулемета. Пришлось прыгать с крыши. Цоя жестко цепануло очередью. С трудом друганы добрались до ДелиМерса.\nНа улице дождь, ДелиМерс мчится на полном ходу, Цой истикает кровью.";
	...
	internal class ThirdActionDeliMersPoint : ThirdAction
	{
		...
		private void DoAfterActionLocal(Player player)
		{
			Console.WriteLine($"{Player.Instance.Name} шарит по карманам в поиске стимулятора, ничего нет, конечно же это же не какой-то там шутер от первого лица, тут все по серьезному. {Player.Instance.Name} нашол в машине аптечку но бинтами и антисептиком пулевую рану не вылечишь... Цой медлено умирает на руках {Player.Instance.Name}а. Перед смертью Цой приказал {Player.Instance.Name}у жить долго и счастливо.\nДелиМерс подьехал к мотелю. {Player.Instance.Name} долго не хотел вылезать из авто, но в итоге решился.");
		}
	...
	internal class FirstActionDeliMersPoint : FirstAction
	{
        ...
        {
			IsAvailable = false;
			MassageAfterAction = $"Цой: Прости братка, лучше бы я остался чилить дома. Передай той девушке с ресепшена, что я любил ее.\n{Player.Instance.Name}: Не мели ерунду экономь силы.\n";
        }
    }
}
