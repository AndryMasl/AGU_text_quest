...
namespace Points
{
	...
	internal class FifthActionMotelPoint : FifthAction
	{
		...
		private void DoAfterActionLocal(Player player)
		{
			...
			Console.WriteLine($"У Трила от злости начинает идти пена из рат, у него закатываются глаза, он падает, его трясет. Вскоре Трил Декстор-Пил вытягивает ноги и испускает последний дух, Трил умер. {Player.Instance.Name} разводит руками, ничего не поделаешь, он был слишком толстый, этого следовало ожидать.\n{Player.Instance.Name} садится на кортаны и достает из кармана био-чип.\n");
		}
