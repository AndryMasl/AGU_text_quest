...
namespace Points
{
    ...
    {
        public override string Content => $"Приехав на адрес {Player.Name} видит странную надпись на стене. 'Гадание , в том числе на кофейном зерне'. Зайдя в здание , там была кофейня. {Player.Instance.Name} вспоминает , что не любит нормис кофе и засрал его когда-то. Подойдя к стойке бара он увидел гадалку Торопкина" +
            $"'О это вы' - сказал Торопкин, 'Мне надо спасти сына и получить оставшиеся буквы' - сказал {Player.Instance.Name}. Они присели на стол. Торопкин разложил карты ТАРО. 'По картам я вижу, что тебе надо убить некого Никиту Белик, который живет в Лос Сантос , рядом с тимкой'" ;
    ...
    internal class ThirdActionInterrogationPoint112 : ThirdAction
    {
        ...
        {
            IsAvailable = false;
            MassageAfterAction = $"{Player.Instance.Name} ждет , пока Торопкин намешает протеина. 'Протеин у вас хороший , жаль что на воде( , Я фанат на молоке' - сказад {Player.Instance.Name} и выкидывает шейкер на пол";
        }
    }
}
