...
namespace Points
{
    ...
    {
        public override string Content => $"Путинцева схватила какой то серп. Кузнецова начала бежать на {Player.Instance.Name}а. А Тольна просто говорила - 'Убейте его', {Player.Instance.Name} немного отошел назад и дверь кабинета сразу закрылась. Рядом был рычаг, который явно поворачивался и открывал дверь. Около рычага лежала какая-то баночка с наклейкой креста...";
    ...
    internal class SecondActionInterrogationPoint127 : SecondAction
    {
        ...
        {
            IsAvailable = false;
            MassageAfterAction = $"{Player.Instance.Name} побежал на Путинцеву и проскачил под ней. Он оказался прямо около Тольны. Тольна оттолкнула {Player.Instance.Name}а обратно к двери";
        }
    }
