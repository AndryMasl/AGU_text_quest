...
namespace Points
{
    ...
    {
        public override string Content => $"Весь бар встает в окне появляется очень яркий свет. Пришло озарение всему этому бару. 'А давайте поднимем бокал за Атомик' - кто-то крикнул из зала. {Player.Name} открывает клинское пивко и видит там на кольце от банки букву 'О'.Подняв голову перед собой, {Player.Name} видит качка. Качок видит {Player.Name}а. Качок:'Поздравляю, ты прошел задание и бьет {Player.Instance.Name}а по грудаку, так сильно, что {Player.Name} отлетает обратно в город Г'";
