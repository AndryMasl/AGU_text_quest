...
namespace Points
{
    ...
    {
        public override string Content => $"В коробке лежла какая-то непонятная Неваляшка. {Player.Name} взял эту хрень и случайно уронил , оттуда выпал микропроект и сразу начал проецировать на стену. {Player.Name} смотрит на стену, а там роботы из атомика какают на стену в сторону {Player.Name}а. {Player.Name} вспомнил атомик и задумался , как бы поступил майор Нечевин(Нечаяв(Майор(Атомик)))?";
        ...
        private void DoBeforeActionLocal()
        {
            ...
            Console.WriteLine($"СильверГрэг вызывает GoldDre и хвастает моментом в проге про атомик и говорит, что Биошок говно и нет его в проге.\n");
        }
    }
